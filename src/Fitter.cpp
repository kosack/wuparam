 ...
 */
double MCSpectrum::weight(double energy, double N0, double Gamma0, double E0,
			  double Time) {
    
    // that's the number of events per time area and energy that we
    // want to have
    //double soll = N0*pow(energy,Gamma0)*Time;
    double soll = N0*pow(energy,Gamma0)*Time*exp(-1*energy/E0);
 ...
 * \todo: this should really take just energy as an argument, and
 * return a MCBinInfo struct!
 */
...
double MCSpectrum::chiSquare( std::vector<Data> &data ) {
	...
	if (data[i].onOffVariance> 0 /* EnergySpectrum::MIN_COUNTS*/) {
...
minimize( EnergySpectrum* energy_on,EnergySpectrum* energy_off, 
	...
	_mcSpectrum->fill(n0,_results.gamma0,e0,Time);
	chiSquare = _mcSpectrum->chiSquare(_data);
	...
	prog2.print(1);
	_mcSpectrum->fill(_results.n0,g,e0,Time);
	chiSquare = _mcSpectrum->chiSquare(_data);
...
void Fitter::save() {
    ...
    cout << "E0          :  " << _results.e0  << endl;
    cout << endl << endl;
    cout << " N0 low_error  high_error gamma0 low_error  high_error "  << endl;
	cout << _results.n0 << "  " << _results.n0M << "  "  << _results.n0P << "  "  
	     << _results.gamma0 << "  " << _results.gammaM << "  " << _results.gammaP  << endl; 
	...
	if (model>0) {
	    ...
	    fluxModel     = _results.n0*pow(energy,_results.gamma0)*exp(-1*energy/_results.e0);
	    fluxObserved  = scaleFactor * fluxModel;
	    
	    if (_data[i].onOffVariance>0)
		error         = sqrt(_data[i].onOffVariance)/_data[i].onOff  
		    * fluxObserved;
	    else
		error =0.;
	}
